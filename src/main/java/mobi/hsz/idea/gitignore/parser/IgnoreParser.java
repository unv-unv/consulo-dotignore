...
public class IgnoreParser implements PsiParser {
  ...
  public void parseLight(IElementType t, PsiBuilder b, LanguageVersion v) {
    ...
    else if (t == ENTRY_DIRECTORY) {
      r = ENTRY_DIRECTORY(b, 0);
    }
    else if (t == ENTRY_FILE) {
      r = ENTRY_FILE(b, 0);
    }
    else if (t == NEGATION) {
      r = NEGATION(b, 0);
  ...
  /* ********************************************************** */
  // item_ *
  static boolean gitignoreFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "gitignoreFile")) return false;
    int c = current_position_(b);
    while (true) {
      if (!item_(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "gitignoreFile", c)) break;
      c = current_position_(b);
    }
    return true;
  }
